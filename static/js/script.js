...
/**
 * Handle clicks on suggestion elements
...
const resizeInput = debounce(function() {
  ...
  if (this.value.trim() !== '') {
    elements.initialView.classList.add('typing');
    elements.suggestions.classList.add('typing');
  } else {
    elements.initialView.classList.remove('typing');
    elements.suggestions.classList.remove('typing');
  }
...
const initEventListeners = () => {
  // DOM content loaded
  document.addEventListener('DOMContentLoaded', attachSuggestionListeners);
  ...
  if (elements.newChatIcon) {
    ...
    console.warn("New chat icon element not found in the DOM.");
  }
